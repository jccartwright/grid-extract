            ...
            _dragEventHandler: function() {
                ...
                this._dragHandler = this.mapView.on('drag', function (e) {
                    ...
                    if (e.action === 'start') {
                        ...
                        let extent = webMercatorUtils.webMercatorToGeographic(this.extentGraphic.geometry)
                        // coords = [geom.xmin, geom.ymin, geom.xmax, geom.ymax].map(x => x.toFixed(4));
                        // console.log(coords);

                        topic.publish(this.topicName, extent);
